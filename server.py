...
def jwt_required(fn):
    def wrapper(*args, **kwargs):
        ...
        if not auth.startswith('Bearer '): return jsonify({'message': 'Token ausente'}), 401
        token = auth.split(' ')[1]
        try:
...
def register_user():
    ...
    try:
        ...
        conn.commit(); return jsonify({'message': 'Criado!'}), 201
...
def login_user():
    ...
    if user and check_password_hash(user['password_hash'], data.get('password')):
        ...
        token = jwt.encode({'sub': str(user['id']), 'username': user['username'], 
...
def get_me():
    ...
    conn = get_db(); c = conn.cursor()
    # Mostra o maior recorde pessoal (Recorde da melhor partida)
    c.execute('SELECT MAX(score) as best FROM scores WHERE user_id = %s', (p['sub'],))
    row = c.fetchone(); c.close(); conn.close()
    return jsonify({'username': p['username'], 'best_score': row['best'] if row['best'] else 0}), 200

@app.route('/leaderboard', methods=['GET'])
def leaderboard():
    conn = get_db(); c = conn.cursor()
    # Soma total para Aritmética
    c.execute("SELECT username, SUM(score) as total FROM scores WHERE modality = 'arithmetic' GROUP BY username ORDER BY total DESC LIMIT 10")
    arith = c.fetchall()
    # Soma total para Álgebra
    c.execute("SELECT username, SUM(score) as total FROM scores WHERE modality = 'algebra' GROUP BY username ORDER BY total DESC LIMIT 10")
    alge = c.fetchall()
    c.close(); conn.close()
    return jsonify({'arithmetic': arith, 'algebra': alge}), 200
